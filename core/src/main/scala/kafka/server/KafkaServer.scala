  ...
  override def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        // 元数据缓存类。该类保存了当前集群上的主题分区详细数据和 Broker 数据。每台 Broker 都维护了一个 MetadataCache 实例。
        // Controller 通过给 Broker 发送 UpdateMetadataRequest 请求的方式，来异步更新这部分缓存数据。
        ...
        // handler-pool numIoThreads默认线程数为8
