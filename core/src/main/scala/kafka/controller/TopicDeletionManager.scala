...
class TopicDeletionManager(config: KafkaConfig,
  ...
  private def retryDeletionForIneligibleReplicas(topics: Set[String]): Unit = {
    ...
    // 通过将对应主题副本的状态，从 ReplicaDeletionIneligible 变更到 OfflineReplica 来完成的
